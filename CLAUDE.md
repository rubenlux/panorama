...
### SPEC 014 — Editorial Analytics Consolidation & Data Quality (2026-07-06/07)

**Objetivo:** hacer confiable y coherente el sistema analítico que Panorama ya posee (`pixel_events` → `src/routes/analytics_v2.js` → CMS Dashboard/ArticleAnalytics). No se agregó tracking nuevo — se corrigió captura y semántica hasta que las métricas fueran matemáticamente coherentes. Precedido por [[pixel-telemetry-audit-2026-07-06]] (SPEC 014 ítem 1, read-only).

**Event Contract V1** (`KNOWN_EVENTS` en `src/routes/pixel.js`) — los 13 eventos reales, cada uno con su cardinalidad documentada: `page_view` (1×ruta, sitio completo), `content_view` (1×vista de artículo, deduped), `time_on_content` (N×heartbeat 20s), `scroll_depth` (≤4×vista), `exit_intent` (≤1×vista), `internal_link_click`, `content_loaded`, `like_click`, `share_click`, `comment_submit`, `engagement`, `ad_impression`, `ad_click`. `validateEvent()` rechaza cualquier nombre fuera de esta lista (antes aceptaba cualquier string).

**`pixel_events.article_id UUID`** — columna real, reemplaza el `OR` defensivo (`payload->>'article_id' = $1 OR payload->>'content_id' = $1`) que existía porque dos clientes drift-earon el mismo concepto a dos nombres de campo. Poblada en el INSERT vía `extractArticleId()` — nunca un cast directo, valida forma UUID antes de castear. Backfill histórico vía `scripts/migrate_pixel_events_article_id.mjs` (auditó antes de migrar: 100% de los valores no-nulos ya eran UUID-shaped).

**Bugs de captura reales encontrados leyendo el código, no solo el SQL** (`web/src/utils/pixel.js`, `useArticleTracking.js`, `App.jsx`):
- `page_view` se disparaba DOS veces por vista de artículo (una vez desde `App.jsx`'s efecto por-`location`, cubre todo el sitio; otra vez desde `useArticleTracking.js` al montar el artículo) — el denominador de cualquier métrica por-artículo estaba roto antes de tocar SQL. Se quitó el segundo disparo.
- El mismo efecto de `App.jsx` se duplicaba TAMBIÉN en dev por el doble-invoke de `React.StrictMode` (sin guardia, a diferencia de `content_view` que sí tiene `hasViewed.current`) — probablemente la causa real de los picos de números redondos vistos en la auditoría original (tráfico de pruebas locales). Fix: guardia por `location.key`.
- `initExitIntent()` podía apilar listeners `mouseleave` sin remover al navegar entre artículos, y los que disparaban leían `this.context` mutable — podían atribuirse al artículo equivocado. Ahora recibe `articleId` por closure, se limpia en el cleanup del efecto (`Pixel.stopExitIntent()`), dedupe con guardia propia.
- `captureNavLinks()` usaba `link.href.includes(hostname)` (substring, falso positivo posible) y no filtraba `href="#"` — confirmado en vivo con el chip de búsqueda de `App.jsx`. Ahora usa `new URL().hostname` real.
- Migración de `visitor_id` (`news_visitor_id` → `pixel_vid`) vivía solo en `SmartAdBanner.jsx`; ahora vive en `PixelService._loadIdentity()`, fuente única.

**Reading Funnel — `eligible_views` + bucketing por `MAX(percent)` por sesión** (`analytics_v2.js`, endpoints `/editorial/article/:id` e `/editorial/insights/engagement`): `views_series` (numerador, 24h) y `scroll_funnel` (antes SIN ventana de tiempo, histórico completo) tenían ventanas distintas — la causa raíz de que el embudo pudiera superar 100% o mostrar abandonos negativos. Fix: `eligible_views = COUNT(DISTINCT session_id)` de `content_view` (no `page_view` — es señal de ruta, no de artículo), y el funnel cuenta sesiones cuyo `MAX(percent)` alcanzó cada umbral, restringido a `eligible_sessions`, misma ventana de 24h. Garantiza `scroll_100<=scroll_75<=scroll_50<=scroll_25<=eligible_views` por construcción, no por suerte del orden de disparo del cliente.

**Traffic Attribution multi-hostname** (`analytics_v2.js`, `OWN_HOSTNAMES`) — lista configurable (`PUBLIC_SITE_HOSTNAMES` env var, CSV) + `localhost`/`127.0.0.1` siempre incluidos, normalización de hostname/puerto (lowercase, sin `www.`, sin puerto) aplicada tanto al referrer como a la lista. `insights/traffic` e `insights/flow` reclasifican como `'Direct'` cualquier referrer propio — antes el tráfico de pruebas locales aparecía como `'Referral'`.

**`time_on_content`** — `SUM(seconds)` por `session_id` primero (evita que un visitante con 2 pestañas/sesiones infle el total), luego `total_engaged_time` / `avg_engaged_time` / `median_engaged_time` (`PERCENTILE_CONT`). **`exit_intent`** — `COUNT(DISTINCT session_id)`, no eventos crudos (el dashboard lo etiquetaba "Abandonos prematuros" cuando medía disparos de listener, no abandonos confirmados).

**Deuda técnica de la auditoría original, resuelta:** `fs.appendFileSync` (I/O síncrona en el hot path de `POST /pixel/events`) eliminado. `email_hash: btoa(email)` en `Newsletter.jsx` eliminado — `btoa` es Base64 reversible, no un hash; el evento de pixel no necesita el email en absoluto.

**Track B (legado) — productores eliminados, tablas NO borradas todavía (a propósito):** `POST /analytics/track` desmontado de `app.js` (0 llamadores confirmados dos veces, con backup previo vía `scripts/backup_track_b_analytics.mjs`); el `INSERT INTO article_stats` de `articles.js` quitado. Las tablas `article_stats` (20 filas) y `events` (347 filas) siguen existiendo, inertes — el `DROP TABLE` queda para un commit independiente posterior, después de validar en producción real. `src/routes/analytics.js` queda en el repo sin montar, no borrado.

**Tests de invariantes con datos sintéticos** — `scripts/test_analytics_invariants.mjs` (patrón `scripts/test_*.mjs` existente, sin framework nuevo): siembra sesiones sintéticas marcadas con un `run_id` único (limpieza garantizada incluso si el script falla), incluyendo el caso adversarial de sesiones que disparan solo el umbral de scroll más profundo alcanzado (expone la diferencia entre `COUNT(*)` crudo por umbral y el bucketing correcto por `MAX(percent)`). Verifica las 7 invariantes: `scroll_100<=scroll_75<=scroll_50<=scroll_25<=eligible_views`, `unique_users<=sessions<=page_views`, `CTR∈[0,1]`, `retention∈[0,1]`, `dropoff>=0`, `internal_destination != '#'`, `own_hostname != acquisition_source`. Correr de nuevo ante cualquier cambio futuro a una agregación de `pixel_events`.

**Siguiente paso estratégico (no iniciado, fuera de este SPEC):** SPEC 015 — Editorial Performance & Goals. Usa las mismas pantallas ya existentes, agrega la capa de comparación/diagnóstico/recomendación que falta (rendimiento relativo a la mediana de categoría, diagnóstico automático, recomendación editorial, seguimiento posterior).
